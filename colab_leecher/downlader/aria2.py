...
def _safe_url(url: str) -> str:
    """
    Encode characters that break aria2c when passed as a list arg.
    Brackets [ ] are the main culprit (HTTP 416 / return code 22).
...
async def aria2_Download(link: str, num: int):
    ...
    name_d = get_Aria2c_Name(safe_link)
    ...
    if exit_code != 0:
        ...
        msg = err_msgs.get(exit_code,
              f"aria2c failed (code {exit_code}) for {link}.\nError: {error_output}")
        logging.error(msg)
...
def get_Aria2c_Name(link: str) -> str:
    ...
    if not name:
        # Fallback: extract from URL path before query string
